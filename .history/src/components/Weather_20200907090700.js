...
const Weather = () => {
    ...
    const handleSubmit = (event) => {
        event.preventDefault();
        let urlInput = `https://api.openweathermap.org/data/2.5/weather?q=${location}&units=${unit}&appid=${API_KEY}`
        setUrl(urlInput)
    };
        ...
        <div id="app">
            <header id="header">
                ...
                <div className="toggle">
                    {unit === "imperial" ? (
                        <div className="toggleBtn imperial selected" onClick={handleF}>&#176;F</div>
                    ) : (
                            <div className="toggleBtn imperial" onClick={handleF}>&#176;F</div>
                        )}
                    {unit === "metric" ? (
                        <div className="toggleBtn metric selected">&#176;C</div>
                    ) : (
                            <div className="toggleBtn metric">&#176;C</div>
                        )}
                </div>
            </header>
                ...
                <div id="results-card">
                    ...
                    <div className="row">
                        <div className="temp-container">
                            <div className="temp-main">{temp}&#176;{unitState == "imperial" ? 'F' : 'C'}</div>
                            <div className="feel temp">Feels like {feel}&#176;{unitState == "imperial" ? 'F' : 'C'}</div>
                            <div className="max temp">High: {max}&#176;{unitState == "imperial" ? 'F' : 'C'}</div>
                            <div className="min temp">Low: {min}&#176;{unitState == "imperial" ? 'F' : 'C'}</div>
